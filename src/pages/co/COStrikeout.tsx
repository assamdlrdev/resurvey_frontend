...
const COStrikeout: React.FC = () => {
    ...
    useEffect(() => {
        if(location.pathname == '/co-strikeout') {
            getCaseData();
        }
    }, [location]);
    ...
    const getCaseData = async () => {
        ...
        setLoading(true);
        const response = await ApiService.get('get_co_strikeout_cases', JSON.stringify(data));
        setLoading(false);
                                    ...
                                    <tbody className="overflow-y-auto">
                                        {(caseData && caseData.length > 0) ? (
                                           caseData.map((cdata, index) => <tr key={index}>
                                            <td className="px-2 py-2 border text-center">{cdata.misc_case_no}</td>
                                            <td className="px-2 py-2 border text-center">{cdata.misc_case_petition_no}</td>
                                            ...
                                            <td className="px-2 py-2 border text-center">
                                                {cdata.status == 0 ? (<Link to={`/co-strikeout-details?case_no=${cdata.misc_case_no}`} className="bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-4 rounded">Proceed</Link>) : `Order Passed`}
                                            </td>
                                           </tr>)
                                        ) : (
                                            <tr>
